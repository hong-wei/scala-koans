...
class AboutImplicits extends KoanSuite with ShouldMatchers {
  ...
  koan("""Implicits wrap around existing classes to provide extra functionality
    ...
    19.isOdd should be(true) // 19.isOdd, first find the method in Int class, if there is Nothing, than find it in current class.
    20.isOdd should be(false)//   Look for the implicit method.
  }
