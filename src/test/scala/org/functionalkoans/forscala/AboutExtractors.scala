...
class AboutExtractors extends KoanSuite {
  ...
  koan(
    ...
    val camaro: Car = new Car("Chevy", "Camaro", 1978, 122)
    val camaro2: Car = new Car("1", "2", 1978, 122)
    val dog = new Dod

    val result = camaro match {
        // camaro2 can be any stable object, including instantiated classes with an unapply method.
      case dog(make, model) => "make: %s, model: %s".format(make, model)
     // case camaro(make, model) => "make: %s, model: %s".format(make, model) // correct
      //case camaro2(make, model) => "make: %s, model: %s".format(make, model) //correct
      case _ => "unknown"
    }
