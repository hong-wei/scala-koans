...
class AboutHigherOrderFunctions extends KoanSuite {
  ...
  koan("Meet lambda. Anonymous function") {
    def lambda:  Function1[Int, Int] = { x: Int => x + 1 }
    def lambda2: Function1[Int, Int] = (x:Int) => x + 1
    val lambda3: Function1[Int, Int] = (x:Int) => x + 1

    val lambda4: Function1[Int, Int] = new Function1[Int, Int] {
      def apply(v1: Int): Int = v1 + 1
    }

    def lambda5(x:Int): Int = x + 1
  ...
  koan("We can take that closure and throw into a method and it will still hold the environment") {
    ...
    var incrementer = 3
    def closure: (Int) => Int = (x: Int) => x + incrementer
    ...
    result2 should be(10+4)
  }

  koan("function returning another function") {
    def addWithoutSyntaxSugar(x: Int): (Int) => Int = {
  ...
  koan("function returning another function " +
    ...
    def addWithSyntaxSugar(x: Int): (Int) => Int = (y:Int) => x + y
  ...
  koan("Currying is a technique to transform function with multiple parameters to function with one parameter") {
    ...
    val function: (Int, Int) => Int = multiply _
  ...
  koan("Currying allows you to create specialized version of generalized function") {
    ...
    def onlyEven(x: Int): Boolean = x % 2 == 0
